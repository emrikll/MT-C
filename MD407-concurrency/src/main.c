...
int main(void) {
  ...
  for (int i = 0; i < CAPACITY_HIGH; i++) {
    xTaskCreateStatic(high_priority_task, "High priority", STACK_SIZE, NULL, 3,
                      xStackHigh[i], &xTaskBufferHigh[i]);
  }

  for (int i = 0; i < CAPACITY_MEDIUM; i++) {
    xTaskCreateStatic(medium_priority_task, "Medium priority", STACK_SIZE, NULL,
                      2, xStackMedium[i], &xTaskBufferMedium[i]);
  }

  for (int i = 0; i < CAPACITY_LOW; i++) {
    xTaskCreateStatic(low_priority_task, "Low priority", STACK_SIZE, NULL, 1,
                      xStackLow[i], &xTaskBufferLow[i]);
  }
